...
from plots.helpers import accuracy, identification_stats, mask, roc_pr_stats, read_llc
import numpy as np
...
class Objective4(Objective):
    ...
    def __call__(self, trial):
        ...
        for i in range(self.n_models):
            ...
            Simulations.run(**self.configs[i], models=[i], reg=reg, alpha_llc=alpha)

        # Calculate an objective value
        roc_auc = Objective4.metric(self.path, self.configs[0]['n_samples'], z)
    ...
    def metric(path, sample_sizes, z):
        ...
        for model in models:
            for edge_type, experiment, sample_size in product(edge_types, experiments, sample_sizes):
                ...
                A_pred = read_llc(dir_path, edge_type, mode=2, faithful='f', sep='')
